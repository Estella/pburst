...
const int stdinSAY::Execute(Packet ePacket) {
  string eTarget;			// target
  string eMessage;			// message

  aServer->writeLog(PBURST_LOG_DEBUG, "stdinSAY::Execute> /SAY received.");
  ...
  aServer->Send("%s P %s :%s", (PBURST_CONFIG_SERVER_NUMERIC).c_str(), eTarget.c_str(), eMessage.c_str());
  return 1;
} // stdinSAY::Execute
