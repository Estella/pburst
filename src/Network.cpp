...
const int Connection::compressBuffer(Bytef *compressFrom, const int compressFromSize) {
  ...
  if (myPacketCount > NETWORK_COMPRESSION_REPORTING) {
    aServer->writeLog(PBURST_LOG_DEBUG, "Connection::compressBuffer> I(%li), O(%li), %d%% ratio.", myCompStream.total_in, myCompStream.total_out, (int) (((double) myCompStream.total_out / (double) myCompStream.total_in) * 100));
    myPacketCount = 0;
  } // if
...
const int Connection::deCompressBuffer(Bytef *deCompressFrom, const int deCompressFromSize, string &deCompressTo) {
  ...
  myPacketCount++;
  if (myPacketCount > NETWORK_COMPRESSION_REPORTING) {
    aServer->writeLog(PBURST_LOG_DEBUG, "Connection::deCompressBuffer> I(%li), O(%li), %d%% ratio.", myDecompStream.total_in, myDecompStream.total_out, (int) (((double) myDecompStream.total_in / (double) myDecompStream.total_out) * 100));
    myPacketCount = 0;
  } // if
...
Connection *Network::addConnection(const string addIP, const string addPort, const string addLocalIP, const string addDescription, const bool addBinary) {
  ...
  if (setsockopt(newFd, SOL_SOCKET, SO_RCVBUF, &setSendSize, sizeof(setSendSize)) < 0 ||
    ...
    aServer->writeLog(PBURST_LOG_DEBUG, "Network::addConnection[setsockopt]> %s", hstrerror(h_errno));
    return NULL;
  } // if
  ...
  if (getsockopt(newFd, SOL_SOCKET, SO_SNDBUF, (char *) &sendSize, &optlen) < 0) {
    aServer->writeLog(PBURST_LOG_DEBUG, "Network::addConnection[getsockopt]> %s", hstrerror(h_errno));
    return NULL;
  } // if
...
const bool Network::removeConnection(const int removeFd) {
  ...
  if (tConnection->getFd() != -1)
    close(tConnection->getFd());

  aServer->writeLog(PBURST_LOG_NORMAL, "*** Disconnected from %s (%s)", tConnection->getIP().c_str(), tConnection->getPort().c_str());
...
const int Network::makeConnection(const string &makeIP, const string &makePort, const string &makeLocalIP) {
  ...
  if ((hp = gethostbyname(connectTo.c_str())) == NULL) {
    aServer->writeLog(PBURST_LOG_DEBUG, "Network::makeConnection[gethostbyname]> %s", hstrerror(h_errno));
    return NETWORK_ERROR;
  } // if
  ...
  if ((newFd = socket(hp->h_addrtype, SOCK_STREAM, 0)) < 0) {
    aServer->writeLog(PBURST_LOG_DEBUG, "Network::makeConnection[socket]> %s", strerror(errno));
    return NETWORK_ERROR;
  } // if
  ...
  if (makeLocalIP.length() > 0) {   
    ...
    if (bind(newFd, (struct sockaddr *) & la, sizeof(la)) == -1) {
      close(newFd);
      aServer->writeLog(PBURST_LOG_DEBUG, "Network::makeConnection[bind]> %s", strerror(errno));
      return NETWORK_ERROR;
    } // if
  }   // if
  ...
  if (connect(newFd, (struct sockaddr *) &sa, sizeof(sa)) < 0) {
    close(newFd);
    aServer->writeLog(PBURST_LOG_DEBUG, "Network::makeConnection[connect]> %s", strerror(errno));
    return NETWORK_ERROR;
  } // if
  ...
  if (fcntl(newFd, F_SETFL, O_NONBLOCK) == -1) {
    close(newFd);
    aServer->writeLog(PBURST_LOG_DEBUG, "Network::makeConnection[fcntl]> %s", strerror(errno));
    return NETWORK_ERROR;
  } // if

  // aServer->writeLog(PBURST_LOG_NORMAL, "Connect[%s] %s:%s", inet_ntoa(*((struct in_addr *) hp->h_addr_list[0])), makeIP.c_str(), makePort.c_str());
  aServer->writeLog(PBURST_LOG_NORMAL, "*** Connecting to %s (%s)", makeIP.c_str(), makePort.c_str());
...
const int Network::makeSocket(const int makePort, const int maxConnections) {
  ...
  if (newFd == -1) {
    aServer->writeLog(PBURST_LOG_DEBUG, "Network::makeSocket[socket]> %s", strerror(errno));
    return NETWORK_ERROR;
  } // if
  ...
  if (setsockopt(newFd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) == -1) {
    close(newFd);
    aServer->writeLog(PBURST_LOG_DEBUG, "Network::makeSocket[setsockopt]> %s", strerror(errno));
    return NETWORK_ERROR;
  } // if
  ...
  if (returnResult == -1) {
    aServer->writeLog(PBURST_LOG_DEBUG, "Network::makeSocket[bind]> %s", strerror(errno));
    return NETWORK_ERROR;
  } // if
  ...
  if (returnResult == -1) {
    aServer->writeLog(PBURST_LOG_DEBUG, "Network::makeSocket[listen]> %s", strerror(errno));
    return NETWORK_ERROR;
  } // if
  ...
  if (fcntl(newFd, F_SETFL, O_NONBLOCK) == -1) {
    close(newFd);
    aServer->writeLog(PBURST_LOG_DEBUG, "Network::makeSocket[fcntl]> %s", strerror(errno));
    return NETWORK_ERROR;
  } // if
...
//    debug(HERE, "Network::acceptConnection[accept]():  errno(%d) errmsg(%s)\n", errno, strerror(errno));
  ...
  if (getsockopt(newFd, SOL_SOCKET, SO_SNDBUF, (char *) &sendSize, &optlen) < 0) {
    aServer->writeLog(PBURST_LOG_DEBUG, "Network::acceptConnection[getsockopt]> %s", hstrerror(h_errno));
    return NETWORK_ERROR;
  } // if
...
const int Network::readConnections() {
  ...
  if (returnResult == -1) {
    aServer->writeLog(PBURST_LOG_DEBUG, "Network::readConnections[select]> %s", strerror(errno));
  ...
  while(ptr != connectionList.end()) {
    ...
    if (FD_ISSET(ptr->second->getFd(), &readSet)) {
      ...
      if (packetLength == -1) {
        aServer->writeLog(PBURST_LOG_DEBUG, "Network::readConnections[read]> %s", strerror(errno));
...
const int Network::writeConnections() {
  ...
  if (returnResult == -1) {
    aServer->writeLog(PBURST_LOG_DEBUG, "Network::writeConnections[select]> %s", strerror(errno));
  ...
  for(ptr = connectionList.begin(); ptr != connectionList.end(); ptr++) {
    ...
    if (FD_ISSET(ptr->second->getFd(), &writeSet)) {
      ...
      while(ptr->second->getWriteBuffer().size() > 0 && writeError == false) {
        packetLength = send(ptr->second->getFd(), ptr->second->getWriteBuffer().c_str(), PBURST_MIN((unsigned) ptr->second->getWriteBuffer().size(), (unsigned) ptr->second->getSendBufferSize()), 0);

        // did we send?
        if (packetLength == -1) {
          aServer->writeLog(PBURST_LOG_DEBUG, "Network::readConnections[send]> %s", strerror(errno));
...
const int Network::sendConnectionPacket(const int sendFd, const string &sendMe, const unsigned int sendLength) {
  ...
  if (returnResult == -1) {
    aServer->writeLog(PBURST_LOG_DEBUG, "Network::sendPacket[send]> %s", strerror(errno));
